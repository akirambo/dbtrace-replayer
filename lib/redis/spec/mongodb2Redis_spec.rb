...
RSpec.describe 'Mongodb TO Redis Unit Test' do
  before do
    ...
    @runner = RedisRunner.new("mongodb", @logger,@options)
  end
  context 'Document(One value) Test' do
    ...
    it "MONGODB_INSERT(simple)/MONGODB_FIND(simple)" do
      ...
      expect(@runner.send("MONGODB_FIND",
          {"key"     => @options[:collection],
            "filter" => {"_id"=>"key00"}})).to eq "val00"
    end
    it "MONGODB_INSERT(bulk)/MONGODB_FIND(simple)" do
      expect(@runner.send("MONGODB_INSERT",
          [[@options[:collection],{"_id"=>"key00","value"=>"val00"},true],
            [@options[:collection],{"_id"=>"key01","value"=>"val01"},true]])).to eq true
      expect(@runner.send("MONGODB_FIND",
          {"key"     => @options[:collection],
            "filter" => {"_id"=>"key00"}})).to eq "val00"
    ...
    it "MONGODB_UPDATE" do
      expect(@runner.send("MONGODB_INSERT",
          [[@options[:collection],{"_id"=>"key00","value"=>"val00"},false]])).to eq true
    ...
    it "MONGODB_DELETE" do
      expect(@runner.send("MONGODB_INSERT",
          [[@options[:collection],{"_id"=>"key00","value"=>"val00"},false]])).to eq true
      expect(@runner.send("MONGODB_INSERT",
          [[@options[:collection],{"_id"=>"key01","value"=>"val01"},false]])).to eq true
    ...
    it "MONGODB_COUNT" do
      expect(@runner.send("MONGODB_INSERT",
          [[@options[:collection],{"_id"=>"key00","value"=>"val00"},false]])).to eq true
      expect(@runner.send("MONGODB_INSERT",
          [[@options[:collection],{"_id"=>"key01","value"=>"val00"},false]])).to eq true
  ...
  context 'Document(multi values) Test' do
    ...
    it "MONGODB_INSERT(bulk)/MONGODB_FIND(simple)" do
      expect(@runner.send("MONGODB_INSERT",
          [[@options[:collection],{"_id"=>"key00","value"=>"val00", "value1"=>"val10"},true],
            [@options[:collection],{"_id"=>"key01","value"=>"val01", "value1"=>"val11"},true]])).to eq true
      ans = [
    ...
    it "MONGODB_UPDATE" do
      expect(@runner.send("MONGODB_INSERT",
          [[@options[:collection],{"_id"=>"key00","value"=>"val00","value1"=>"val10"},false]])).to eq true
      expect(@runner.send("MONGODB_UPDATE",
          {"key" => @options[:collection],
            "query" => {"_id" =>"key00"},
            "update" => {"$set" => { "value" => "valXX"}},
    ...
    it "MONGODB_DELETE" do
      expect(@runner.send("MONGODB_INSERT",
          [[@options[:collection],{"_id"=>"key00","value"=>"val00","value1"=>"val10"},false]])).to eq true
      expect(@runner.send("MONGODB_INSERT",
          [[@options[:collection],{"_id"=>"key01","value"=>"val01","value1"=>"val10"},false]])).to eq true
      expect(@runner.send("MONGODB_DELETE",
          {"key" => @options[:collection], 
            "filter" => {"_id" => "key00"}})).to eq true
      ans = []
    ...
    it "MONGODB_COUNT" do
      expect(@runner.send("MONGODB_INSERT",
          [[@options[:collection],{"_id"=>"key00","value"=>"val00","value1"=>"val10"},false]])).to eq true
      expect(@runner.send("MONGODB_INSERT",
          [[@options[:collection],{"_id"=>"key01","value"=>"val00","value1"=>"val10"},false]])).to eq true
      expect(@runner.send("MONGODB_COUNT",
          {"key" => @options[:collection],
            "filter" => {"_id" => "key00"}})).to eq 1
    end
